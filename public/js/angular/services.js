...
angular.module('kinyelo.services', []).factory('editableContentService', function($window) {
    ...
    var nextNodeDescendants = function(node) {
     ...
     */
    var isAncestor = function(ancestor, descendant) {
        return ancestor
            && descendant
     ...
     * otherwise.
     */
    var isAncestorContainer = function(ancestor, descendant) {
     ...
     */
    var isBefore = function(node1, node2) {
     ...
     */
    var isAfter = function(node1, node2) {
    ...
    var getInclusiveAncestors = function(node) {
    ...
    var getInclusiveDescendants = function(node) {
        return [node].concat(getDescendants(node));
    }

    var convertProperty = function(property) {
...
// is none.
    var cssSizeToLegacy = function(cssVal) {
        return {
            "x-small": 1,
            "small": 2,
            "medium": 3,
...
// Return the CSS size given a legacy size.
    var legacySizeToCss = function(legacyVal) {
        return {
            1: "x-small",
            2: "small",
            3: "medium",
...
// Opera 11 puts HTML elements in the null namespace, it seems.
    var isHtmlNamespace = function(ns) {
        return ns === null
            || ns === htmlNamespace;
    }
...
// list:"
    var getDirectionality = function(element) {
...
//   node's childNodes's length."
     ...
     * DOM Range.
     */
    var getPosition = function(nodeA, offsetA, nodeB, offsetB) {
     ...
     */
    var getFurthestAncestor = function(node) {
        var root = node;
     ...
     */
    var isContained = function(node, range) {
     ...
     */
    var getContainedNodes = function(range, condition) {
     ...
     */
    var getAllContainedNodes = function(range, condition) {
        if (typeof condition == "undefined") {
            condition = function() { return true };
        }
        var node = range.startContainer;
...
// Returns either null, or something of the form #xxxxxx.
    var parseSimpleColor = function(color) {
...
// Helper function for common behavior.
    var editCommandMethod = function(command, range, callback) {
    ...
    var myExecCommand = function(command, showUi, value, range) {
    ...
    var myQueryCommandEnabled = function(command, range) {
        // "All of these methods must treat their command argument ASCII
        // case-insensitively."
        command = command.toLowerCase();
    ...
    var myQueryCommandIndeterm = function(command, range) {
        // "All of these methods must treat their command argument ASCII
        // case-insensitively."
        command = command.toLowerCase();
    ...
    var myQueryCommandState = function(command, range) {
        // "All of these methods must treat their command argument ASCII
        // case-insensitively."
        command = command.toLowerCase();
...
// supported, and false otherwise."
    var myQueryCommandSupported = function(command) {
        // "All of these methods must treat their command argument ASCII
        // case-insensitively."
        command = command.toLowerCase();
    ...
    var myQueryCommandValue = function(command, range) {
        // "All of these methods must treat their command argument ASCII
        // case-insensitively."
        command = command.toLowerCase();
...
// too, like isHtmlElement(node, ["OL", "UL"]) to check if it's an ol or ul.
    var isHtmlElement = function(node, tags) {
...
// prohibited paragraph child name."
    var isProhibitedParagraphChild = function(node) {
...
// of a Document whose designMode is enabled."
    var isEditingHost = function(node) {
        return node
            && isHtmlElement(node)
...
// Helper function, not defined in the spec
    var hasEditableDescendants = function(node) {
...
// node that is an editing host, if node is editable."
    var getEditingHostOf = function(node) {
...
// non-null and the same as the editing host of the second."
    var inSameEditingHost = function(node1, node2) {
...
// else in it, and therefore has zero height."
    var isCollapsedLineBreak = function(br) {
...
// contenteditable.
    var isExtraneousLineBreak = function(br) {
        if (!isHtmlElement(br, "br")) {
            return false;
        }
...
// true:"
    var isCollapsedWhitespaceNode = function(node) {
...
// least one child that is a collapsed block prop."
    var isCollapsedBlockProp = function(node) {
...
// $window.getSelection() on the context object. (Thus the active range may be null.)"
//
...
// descend from a Document.
    var getActiveRange = function() {
        ...
        if (globalRange) {
            ...
            ret = $window.getSelection().getRangeAt(0);
        } else {
            return null;
        }
...
// Magic array of extra ranges whose endpoints we want to preserve.
    ...
    var movePreservingRanges = function(node, newParent, newIndex) {
    ...
    var setTagName = function(element, newName) {
    ...
    var removeExtraneousLineBreaksBefore = function(node) {
    ...
    var removeExtraneousLineBreaksAtTheEndOf = function(node) {
        // "Let ref be node."
        var ref = node;
...
// breaks before it, then remove extraneous line breaks at the end of it."
    var removeExtraneousLineBreaksFrom = function(node) {
...
//@{
    ...
    var wrap = function(nodeList, siblingCriteria, newParentInstructions) {
...
// name of an element with inline contents."
    var isElementWithInlineContents = function(node) {
        return isHtmlElement(node, namesOfElementsWithInlineContents);
    }

    var isAllowedChild = function(child, parent_) {
...
// collapsed, and at least one of the following holds:"
    var isEffectivelyContained = function(node, range) {
        if (range.collapsed) {
            return false;
        }
...
// Like get(All)ContainedNodes(), but for effectively contained nodes.
    var getEffectivelyContainedNodes = function(range, condition) {
        if (typeof condition == "undefined") {
            condition = function() { return true };
        }
        var node = range.startContainer;
    ...
    var getAllEffectivelyContainedNodes = function(range, condition) {
        if (typeof condition == "undefined") {
            condition = function() { return true };
        }
        var node = range.startContainer;
...
// with no attributes except possibly style and/or href."
    var isModifiableElement = function(node) {
    ...
    var isSimpleModifiableElement = function(node) {
...
// an img, or a br."
    var isFormattableNode = function(node) {
...
// values and they match the definition."
    var areEquivalentValues = function(command, val1, val2) {
...
// the corresponding value set ("1" through "7" respectively)."
    var areLooselyEquivalentValues = function(command, val1, val2) {
...
//@{
    ...
    var getEffectiveCommandValue = function(node, command) {
    ...
    var getSpecifiedCommandValue = function(element, command) {
    ...
    var reorderModifiableDescendants = function(node, command, newValue) {
    ...
    var recordValues = function(nodeList) {
...
//@{
    ...
    var clearValue = function(element, command) {
...
//@{
    ...
    var pushDownValues = function(node, command, newValue) {
...
//@{
    ...
    var forceValue = function(node, command, newValue) {
        ...
        if (!cssStylingFlag) {
            ...
            if (command == "bold" && (newValue == "bold" || newValue == "700")) {
                newParent = node.ownerDocument.createElement("strong");
            }
            ...
            if (command == "italic" && newValue == "italic") {
                newParent = node.ownerDocument.createElement("em");
            }
...
//@{
    ...
    var setSelectionValue = function(command, newValue) {
        ...
        if (isEditable(getActiveRange().startContainer)
            ...
            $window.getSelection().removeAllRanges();
            $window.getSelection().addRange(newActiveRange);
...
// if the size is invalid.
    ...
    var getLegacyFontSize = function(size) {
...
//@{
    commands.removeformat = {
        action: function() {
            ...
            // "tt", "u", or "var"."
            var isRemoveFormatCandidate = function(node) {
                return isEditable(node)
...
//@{
    commands.strikethrough = {
        action: function() {
            ...
            return true;
        }, inlineCommandActivatedValues: ["line-through"], relevantCssProperty: null
    };
...
// attribute that sets "margin" or some subproperty of it."
    var isIndentationElement = function(node) {
        if (!isHtmlElement(node)) {
            return false;
        }
...
//   * "a dir attribute."
    var isSimpleIndentationElement = function(node) {
...
// or "xmp"."
    var isNonListSingleLineContainer = function(node) {
...
// HTML element with local name "li", "dt", or "dd"."
    var isSingleLineContainer = function(node) {
        return isNonListSingleLineContainer(node)
            || isHtmlElement(node, ["li", "dt", "dd"]);
    }

    var getBlockNodeOf = function(node) {
...
//@{
    ...
    var fixDisallowedAncestors = function(node) {
    ...
    var normalizeSublists = function(item) {
    ...
    var getSelectionListState = function() {
    ...
    var getAlignmentValue = function(node) {
    ...
    var getNextEquivalentPoint = function(node, offset) {
    ...
    var getPreviousEquivalentPoint = function(node, offset) {
        // "If node's length is zero, return null."
        if (getNodeLength(node) == 0) {
            return null;
        }
    ...
    var getFirstEquivalentPoint = function(node, offset) {
    ...
    var getLastEquivalentPoint = function(node, offset) {
...
// 1, and that child is either a visible block node or a visible br."
    var isBlockStartPoint = function(node, offset) {
...
// offset, and that child is a visible block node."
    var isBlockEndPoint = function(node, offset) {
...
// point or a block end point."
    var isBlockBoundaryPoint = function(node, offset) {
        return isBlockStartPoint(node, offset)
            || isBlockEndPoint(node, offset);
    }

    var blockExtend = function(range) {
    ...
    var followsLineBreak = function(node) {
        // "Let offset be zero."
        var offset = 0;
    ...
    var precedesLineBreak = function(node) {
...
//@{
    ...
    var recordCurrentOverrides = function() {
    ...
    var recordCurrentStatesAndValues = function() {
        // "Let overrides be a list of (string, string or boolean) ordered pairs,
        // initially empty."
        var overrides = [];
    ...
    var restoreStatesAndValues = function(overrides) {
        // "Let node be the first formattable node effectively contained in the
        // active range, or null if there is none."
        var node = getAllEffectivelyContainedNodes(getActiveRange())
            .filter(isFormattableNode)[0];
...
// stripWrappers, and/or direction as keys.
    var deleteSelection = function(flags) {
        if (flags === undefined) {
            flags = {};
        }
        ...
        if (isAncestor(startBlock, endBlock)) {
            ...
            // node."
            $window.getSelection().collapse(startBlock, getNodeIndex(referenceNode));
            ...
            $window.getSelection().collapse(startBlock, getNodeLength(startBlock));
...
//@{
    ...
    var splitParent = function(nodeList) {
        // "Let original parent be the parent of the first member of node list."
        var originalParent = nodeList[0].parentNode;
...
// its parent."
    var removePreservingDescendants = function(node) {
        if (node.hasChildNodes()) {
...
//@{
    ...
    var canonicalSpaceSequence = function(n, nonBreakingStart, nonBreakingEnd) {
    ...
    var canonicalizeWhitespace = function(node, offset, fixCollapsedSpace) {
...
//@{
    ...
    var indentNodes = function(nodeList) {
...
//@{
    ...
    var toggleLists = function(tagName) {
...
//@{
    ...
    var justifySelection = function(alignment) {
        // "Block-extend the active range, and let new range be the result."
...
//<let-dig> ::= <letter> | <digit>
    ...
    var autolink = function(node, endOffset) {
    ...
    commands["delete"] = {
        ...
        action: function() {
            ...
            if ((node.nodeType == Node.TEXT_NODE
                ...
                // Selection."
                $window.getSelection().extend(node, offset - 1);
            ...
            if (0 <= startOffset - 1
                ...
                // object's Selection."
                $window.getSelection().collapse(startNode, startOffset - 1);
            ...
            if (isHtmlElement(startNode.childNodes[startOffset], ["li", "dt", "dd"])
                ...
                $window.getSelection().extend(node, 0);
            ...
            $window.getSelection().extend(node, offset);
            getActiveRange().setEnd(node, offset);
    ...
    commands.forwarddelete = {
        ...
        action: function() {
            ...
            if (node.nodeType == Node.TEXT_NODE
                ...
                // Selection."
                $window.getSelection().extend(node, endOffset);
    ...
    commands.inserthorizontalrule = {
        ...
        action: function() {
            ...
            if (getActiveRange().startContainer.nodeType == Node.TEXT_NODE
                ...
                $window.getSelection().collapse(newNode, newOffset);
            ...
            // index."
            $window.getSelection().collapse(hr.parentNode, 1 + getNodeIndex(hr));
...
//@{
    commands.insertlinebreak = {
        ...
        action: function(value) {
            ...
            // argument."
            $window.getSelection().collapse(br.parentNode, 1 + getNodeIndex(br));
...
//@{
    commands.insertparagraph = {
        ...
        action: function() {
            ...
            $window.getSelection().collapse(newContainer, 0);
    ...
    commands.inserttext = {
        action: function(value) {
            ...
            if (node.nodeType == Node.TEXT_NODE) {
                ...
                try { $window.getSelection().extend(node, offset + 1); } catch(e) {}
                getActiveRange().setEnd(node, offset + 1);
...
//@{
    commands.selectall = {
        ...
        action: function() {
            ...
            // "If target is null, call $window.getSelection() on the context object, and
            // call removeAllRanges() on the result."
            if (!target) {
                $window.getSelection().removeAllRanges();

                // "Otherwise, call $window.getSelection() on the context object, and call
...
//@}
    return {
        ...
        formattableBlockNames: formattableBlockNames
