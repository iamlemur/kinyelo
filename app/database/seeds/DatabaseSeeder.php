...
class DatabaseSeeder extends Seeder {
	...
	public function run()
	{
		...
		$this->command->info('Characters seeded!');
...
class BookTableSeeder extends Seeder {
	...
	public function run()
	{
		...
		$books = array();
		$books[] = Book::create(array(
			'user_id' => '1',
			'title' => 'The Best Book Ever',
			'summary' => '<p>Is this a formatted summary? I think so.</p>',
			'status' => 'draft'
		));
		...
		$books[] = Book::create(array(
			'user_id' => '2',
			'title' => 'Dinosaurs of the paleomezzoic area.',
			'summary' => '<p>Is this a formatted summary? I think so.</p>',
			'status' => 'draft'
		));

		$posts = array();
		Post::all()->each(function($post) use(&$posts) {
			array_push($posts, $post);
		});
		foreach($books as $book) {
			$book->posts()->saveMany($posts);
		}
...
class CharacterTableSeeder extends Seeder {
	...
	public function run()
	{
		...
		Character::create(array(
			'name' => 'Jimmy Fallon',
			'description' => '<p>I think this will be formatted too.</p>',
			'post_id' => '1'
		));
		Character::create(array(
			'name' => 'Jay Leno',
